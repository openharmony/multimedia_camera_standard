...
public:
    ...
    virtual ~CameraConfigImpl() = default;

    void SetFrameStateCallback(const FrameStateCallback &callback, const EventHandler &handler) override
    {
        if (inUse_) {
            MEDIA_ERR_LOG("This config is in use, do not support config modify.");
        }

        callback_ = (FrameStateCallback *) &callback;
        handler_ = (EventHandler *) &handler;
        inUse_ = true;
    }

    EventHandler *GetEventHandler() const override
    {
        EventHandler *evtHandler = handler_;
        return evtHandler;
    }

    FrameStateCallback *GetFrameStateCb() const override
    {
        FrameStateCallback *fsCb = callback_;
        return fsCb;
    }
