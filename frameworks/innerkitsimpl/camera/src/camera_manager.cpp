...
public:
    ...
    void OnCameraServiceInitialized(const list<string> &availCameraIdList) override
    {
    ...
    void OnCameraStatusChange(const string &cameraId, CameraStauts status, const CameraDevice &device) override
    {
        ...
        switch (status) {
            ...
            case CAMERA_STATUS_CREATED:
                if (p != cameraMapCache_.end()) {
                    p->second->OnCreate((CameraDevice &) device);
                }
                break;
    ...
    void UnregisterCameraDeviceCallback(const CameraDeviceCallback &callback) override
    {
    ...
    void CreateCamera(const string &cameraId, CameraStateCallback &callback, EventHandler &handler) override
    {
        auto p = cameraMapCache_.find(cameraId);
        if (p == cameraMapCache_.end()) {
            MEDIA_FATAL_LOG("Camera impl not found.");
...
private:
    ...
    void InitCameraAbility(const string &cameraId)
    {
        auto ability = cameraService_->GetCameraAbility((string &) cameraId);
        if (ability == nullptr) {
            MEDIA_FATAL_LOG("Camera ability is null.");
            return;
        }

        CameraImpl *cam = new (nothrow) CameraImpl((string &) cameraId, *ability);
        if (cam == nullptr) {
